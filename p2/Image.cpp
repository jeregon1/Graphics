...
Image::Image(const string& PPMfilename){
...
void skipComments(ifstream &file, float &diskColorResolution) {
    while (file.peek() == '#') {
        string comment;
        getline(file, comment);
        if (comment.find("#MAX=") == 0) {
            diskColorResolution = stof(comment.substr(5));
        }
    }
}
...
void Image::writePPM(const string& path, float diskColorResolution, float memoryColorResolution) const {
    ...
    file << (int) diskColorResolution << "\n";

    file << fixed << setprecision(0); // Sin decimales
    float maxColorRatio = diskColorResolution / memoryColorResolution;
    for (int i = 0; i < height; i++) {
        int j = i * width;
        file << round(pixels[j] * maxColorRatio); // First element without \t
...
Image Image::readBMP(const string& path) {
    ...
    for (int y = 0; y < image.height; ++y) {
        for (int x = 0; x < image.width; ++x) {
            ...
            image.pixels[y * image.width + x] = RGB(r / 255.0f, g / 255.0f, b / 255.0f);
        }
        file.ignore(padding);
    }
